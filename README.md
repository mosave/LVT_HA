...
### Ключевые фразы (intents)

```
lvt:
  intents:
    - intent: <intent_id>       # Название ключевой фразы (intent). Допустимо определение несколько интентов с одним и тем же Id
      speaker: <speaker_id>     # Терминал или список терминалов, к которым привязывается данная ключевая фраза (необязательный)
      utterance: <utterance>    # Шаблон или список шаблонов ключевых фраз
      data: <data>              # Значения дополнительных переменных (слотов) (необязательный)
```
...
lvt.negotiate
    ...
    option_1_data: <data>                   # Дополнительные переменные (слоты), возвращаемые при выборе первого варианта (необязательный)
...
```
lvt.restart_speaker
    speaker: <speaker_id>                   # Список терминалов, которые требуется перезагрузить (обновить)
