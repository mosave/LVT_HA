...
- **Удар+е́ние** в слове обозначается символ "+", который ставится непосредственно перед ударной гласной. Кро́ме того́, RHVóice подде́рживает Unicóde
  си́мвол с ко́дом **u0301** (769), который должен следовать **за** ударной гласной.
- Для **согласования и склонения слов** в используется конструкция **[слово или фраза: <список тегов>]**: При этом все слова, находящиеся до доеточия будут преобразованы в соответствии со списком переданных [OpenCorpora тегов](https://pymorphy2.readthedocs.io/en/stable/user/grammemes.html). Допустимо использовать как латинское так и русское название тегов. Если в списке тегов присутствует число, то после обработки тегов OpenCorpora текст будет согласован с этим числом.
...
- "Значение слов может зависеть от ударения. Например з+амок и зам+ок, про́пасть и пропа́сть,запла́чу и заплачу́"

## Язык описания ключевых фраз (utterance)

Шаблон ключевой фразы позволяет достаточно гибко описывать произнесение каких фраз ожидается от человека.
При распознавании по шаблону LVT ограничивает голосовую модель только ожидаемыми словами (если это поддерживается соответствующей модель).
